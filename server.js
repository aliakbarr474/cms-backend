...
// CORS might be needed depending on your frontend setup, uncomment if needed
// app.use(cors()); 
...
app.get('/projects/show', (req, res) => {
  const sql = `
    SELECT 
      ...
      COALESCE(
        (SELECT balance FROM project_ledger pl WHERE pl.project_id = p.project_id ORDER BY pl.project_ledger_id DESC LIMIT 1), 0
      ) AS current_balance
    FROM projects p 
      ...
      db.query(balanceSql, [project_id], (err3, result3) => {
        ...
        db.query(projectLedgerSql, [project_id, 'Payment Received', amount, 0, newBalance], (err4) => {
          ...
          return res.status(200).json({
            ...
            payment_id: result2.insertId, method: method, amount: amount
          });
        });
      });
    });
...
app.get('/ledger/show', (req, res) => {
  const sql = `
    SELECT
      entry_date, description, debit, credit, SUM(credit - debit) OVER (ORDER BY entry_date, ledger_id) AS balance
    FROM ledger
  ...
  db.query(getVendorSql, [vendor], (err, result) => {
    ...
    db.query(invoiceSql, [vendor_id, invoiceNum], (err2, result2) => {
      ...
      return res.status(200).json({
        ...
        insertId: result2.insertId, vendor_id, invoiceNum
      });
    });
  });
});
...
// Catch-all route to hand navigation over to React Router
app.get('*', (req, res) => {
