...
app.post('/login', (req, res) => {
  ...
  db.query(sql, [username, password], (err, result) => {
    ...
    if (result.length === 0) {
      ...
      return res.status(200).json({ message: 'Login Successful', username: user, token: token });
    }
  })
});
...
app.post('/clients/add', (req, res) => {
  ...
  if (!client) return res.status(400).json({ message: 'Client name missing' });

  const sql = 'INSERT INTO clients (client_name,phone) VALUES (?,?)'
  db.query(sql, [client, clientPhone], (err, result) => {
    if (err) return res.status(500).json({ message: 'Database error' });
    return res.status(200).json({
      message: 'Labor added successfully', insertId: result.insertId, client_name: client, phone: clientPhone,
    });
  })
})
...
app.delete('/clients/delete/:id', (req, res) => {
  const { id } = req.params;
  const getProjectsSql = `SELECT project_id FROM projects WHERE client_id = ?`;

  db.query(getProjectsSql, [id], (err1, projects) => {
    if (err1) return res.status(500).json({ message: 'Failed to fetch projects' });

    const projectIds = projects.map(p => p.project_id);

    const deleteClient = () => {
      db.query('DELETE FROM clients WHERE client_id = ?', [id], (errFinal, result) => {
          if (errFinal) return res.status(500).json({ message: 'Client delete failed' });
    ...
    if (projectIds.length === 0) return deleteClient();
    ...
    const runDeletes = (index) => {
      if (index === deleteQueries.length) return deleteClient();
      db.query(deleteQueries[index], projectIds, (errStep) => {
        if (errStep) return res.status(500).json({ message: 'Failed to delete related data' });
        runDeletes(index + 1);
      });
    };
    runDeletes(0);
...
app.post('/labor/add', (req, res) => {
  ...
  if (!labor) return res.status(400).json({ message: 'Labor name missing' });

  const sql = 'INSERT INTO labor (labor_name,salary) VALUES (?,?)'
  db.query(sql, [labor, salary], (err, result) => {
    if (err) return res.status(500).json({ message: 'Database error' });
    return res.status(200).json({
      message: 'Labor added successfully', insertId: result.insertId, labor_name: labor, salary: salary,
    });
  })
})
...
app.delete('/labor/delete/:id', (req, res) => {
  ...
  db.query(sql, [id], (err, result) => {
    if (err) return res.status(400).json({ message: 'Database Error' });
    if (result.affectedRows === 0) return res.status(404).json({ message: 'Labor not found' });
...
app.post('/vendors/add', (req, res) => {
  ...
  if (!vendor) return res.status(400).json({ message: 'Vendor name missing' });

  const opening = Number(openingBalance);
  const insertVendorSql = 'INSERT INTO vendors (vendor_name, phone, opening_balance) VALUES (?,?,?)';
  ...
  db.query(insertVendorSql, [vendor, vendorPhone, opening], (err, result) => {
    ...
    const insertLedgerSql = `INSERT INTO vendor_ledger (vendor_id, description, debit, credit, balance) VALUES (?,?,?,?,?)`;

    db.query(insertLedgerSql, [vendorId, 'Opening Balance', debit, credit, balance], (ledgerErr) => {
        if (ledgerErr) return res.status(500).json({ message: 'Database error' });
        io.emit('vendorAdded');
        return res.status(200).json({
          message: 'Vendor added successfully', insertId: vendorId, vendor_name: vendor, phone: vendorPhone, opening_balance: opening
        });
      }
    );
  });
...
app.get('/vendors/show', (req, res) => {
  const sql = `
    SELECT v.*, COALESCE((SELECT balance FROM vendor_ledger vl WHERE vl.vendor_id = v.vendor_id ORDER BY vl.vendor_ledger_id DESC LIMIT 1), 0) AS current_balance
    FROM vendors v
  `;
  db.query(sql, (err, result) => {
    if (err) return res.status(500).json({ message: 'Database Error' });
...
app.delete('/vendors/delete/:id', (req, res) => {
  const { id } = req.params;
  const getInvoicesSql = `SELECT invoice_id FROM vendor_invoices WHERE vendor_id = ?`;

  db.query(getInvoicesSql, [id], (err1, invoices) => {
    if (err1) return res.status(500).json({ message: 'Failed to fetch vendor invoices' });
    const invoiceIds = invoices.map(inv => inv.invoice_id);

    const deleteVendor = () => {
      db.query('DELETE FROM vendors WHERE vendor_id = ?', [id], (errFinal, result) => {
          if (errFinal) return res.status(500).json({ message: 'Vendor delete failed' });
    ...
    if (invoiceIds.length === 0) {
      db.query('DELETE FROM vendor_ledger WHERE vendor_id = ?', [id], (errLedger) => {
        if (errLedger) return res.status(500).json({ message: 'Vendor ledger delete failed' });
        db.query('DELETE FROM project_vendors WHERE vendor_id = ?', [id], (errPV) => {
          if (errPV) return res.status(500).json({ message: 'Project vendor delete failed' });
          return deleteVendor();
        });
      });
      return;
    ...
    const runInvoiceDeletes = (index) => {
      ...
      db.query(deleteInvoiceChildren[index], invoiceIds, (errStep) => {
        if (errStep) return res.status(500).json({ message: 'Failed to delete invoice data' });
...
app.post('/projects/add', (req, res) => {
  ...
  let client_id = '';
  if (!projectName) return res.status(400).json({ message: 'Project name missing' });
  ...
  db.query(sql1, [clientName], (err1, result1) => {
    ...
    db.query(sql, [client_id, projectName, advance], (err, result) => {
      ...
      db.query(sqlLedger, [projectId, 'Advance', 0, advance, advance], (err2, result2) => {
        ...
        return res.status(200).json({
          message: 'Project & advance added successfully', insertId: projectId, client_name: clientName, project_name: projectName, advance: advance, balance: advance
        });
      })
    })
  })
...
app.get('/projects/show', (req, res) => {
  const sql = `
    SELECT 
      p.project_id, p.project_name, p.advance, c.client_id, c.client_name, c.phone,
      COALESCE((SELECT balance FROM project_ledger pl WHERE pl.project_id = p.project_id ORDER BY pl.project_ledger_id DESC LIMIT 1), 0) AS current_balance
    FROM projects p 
...
app.delete('/projects/delete/:id', (req, res) => {
  ...
  db.query('DELETE FROM project_ledger WHERE project_id = ?', [projectId], (err1) => {
    if (err1) return res.status(500).json({ message: 'Ledger delete failed' });
    db.query('DELETE FROM payments WHERE project_id = ?', [projectId], (err2) => {
      if (err2) return res.status(500).json({ message: 'Payments delete failed' });
      db.query('DELETE FROM projects WHERE project_id = ?', [projectId], (err3) => {
        if (err3) return res.status(500).json({ message: 'Project delete failed' });
...
app.post('/expenses/add', (req, res) => {
  ...
  if (!type || !amount || !method) return res.status(400).json({ message: 'Missing required fields' });

  if (type === 'project') {
    const findProject = `SELECT project_id FROM projects WHERE project_name = ?`;
    db.query(findProject, [project], (err, projectResult) => {
      if (err) return res.status(500).json({ message: 'Database error' });
      if (projectResult.length === 0) return res.status(400).json({ message: 'Project not found' });

      const project_id = projectResult[0].project_id;
      const insertExpense = `INSERT INTO expenses (type, project_id, amount, method) VALUES (?, ?, ?, ?)`;
      ...
      db.query(insertExpense, [type, project_id, amount, method], (err2, result) => {
        ...
        db.query(balanceSql, [project_id], (err3, result3) => {
          ...
          const projectLedgerSql = `INSERT INTO project_ledger (project_id, description, debit, credit, balance) VALUES (?,?,?,?,?)`;
          db.query(projectLedgerSql, [project_id, 'Payment Made', 0, amount, newBalance], (err4) => {
            if (err4) return res.status(500).json({ message: 'Database error' });
            res.json({
              expense_id: result.insertId, type, project_name: project, vendor_name: null, amount, method, date: new Date().toLocaleDateString('en-GB')
            });
          })
        });
      });
  ...
  if (type === 'purchase') {
    ...
    db.query(findVendor, [vendor], (err, vendorResult) => {
      if (err) return res.status(500).json({ message: 'Database error' });
      if (vendorResult.length === 0) return res.status(400).json({ message: 'Vendor not found' });

      const vendor_id = vendorResult[0].vendor_id;
      const insertExpense = `INSERT INTO expenses (type, vendor_id, amount, method) VALUES (?, ?, ?, ?)`;
      ...
      db.query(insertExpense, [type, vendor_id, amount, method], (err2, result) => {
        ...
        db.query(balanceSql, [vendor_id], (err3, result3) => {
          ...
          const newBalance = Number(result3[0].balance) - Number(amount);
          const vendorLedgerSql = `INSERT INTO vendor_ledger (vendor_id, description, debit, credit, balance) VALUES (?,?,?,?,?)`;
          db.query(vendorLedgerSql, [vendor_id, 'Payment Made', 0, amount, newBalance], (err4) => {
            if (err4) return res.status(500).json({ message: 'Database error' });
            io.to(`vendor-${vendor_id}`).emit('vendor-ledger-updated')

            res.json({
              expense_id: result.insertId, type, project_name: null, vendor_name: vendor, amount, method, date: new Date().toLocaleDateString('en-GB')
            });
          })
        })
      });
...
app.get('/expenses/show', (req, res) => {
  const sql = `
    SELECT
      ...
      e.amount, e.method, DATE_FORMAT(e.added_at, '%d/%m/%Y') AS date
    FROM expenses e
...
app.delete('/expenses/delete/:id', (req, res) => {
  ...
  db.query('DELETE FROM ledger WHERE reference_type = ? AND reference_id = ?', ['EXPENSE', id], (err1) => {
    if (err1) return res.status(500).json({ message: 'Failed to delete related ledger entries' });
    db.query('DELETE FROM expenses WHERE expense_id = ?', [id], (err2, result) => {
      if (err2) return res.status(500).json({ message: 'Expense delete failed' });
...
app.post('/payments/add', (req, res) => {
  ...
  if (!project || !amount) return res.status(400).json({ message: 'Project or amount missing' });

  const sqlProject = `SELECT project_id FROM projects WHERE project_name = ?`;
  db.query(sqlProject, [project], (err, projectResult) => {
    if (err) return res.status(500).json({ message: 'Database error' });
    if (projectResult.length === 0) return res.status(404).json({ message: 'Project not found' });
    ...
    db.query(sql, [project_id, method, amount], (err2, result2) => {
      ...
      db.query(balanceSql, [project_id], (err3, result3) => {
        ...
        db.query(projectLedgerSql, [project_id, 'Payment Received', amount, 0, newBalance], (err4) => {
          if (err4) return res.status(500).json({ message: 'Database error' });
          return res.status(200).json({
            message: 'Payment & ledger updated successfully', payment_id: result2.insertId, method: method, amount: amount
          });
        });
      });
    });
...
app.delete('/payments/delete/:id', (req, res) => {
  ...
  db.query('DELETE FROM ledger WHERE reference_type = ? AND reference_id = ?', ['PAYMENT', id], (err1) => {
    ...
    db.query('DELETE FROM payments WHERE payment_id = ?', [id], (err2, result) => {
      if (err2) return res.status(500).json({ message: 'Payment delete failed' });
...
app.get('/ledger/show', (req, res) => {
  const sql = `
    SELECT entry_date, description, debit, credit, SUM(credit - debit) OVER (ORDER BY entry_date, ledger_id) AS balance
    FROM ledger ORDER BY entry_date, ledger_id
  `;
  ...
  db.query(getVendorSql, [vendor], (err, result) => {
    ...
    db.query(invoiceSql, [vendor_id, invoiceNum], (err2, result2) => {
      ...
      io.emit('invoiceAdded', {
        invoice_id: result2.insertId, vendor_id, invoice_number: invoiceNum, status: 'DRAFT', total_amount: 0, advance_paid: 0, balance: 0
      });

      return res.status(200).json({
        message: 'Invoice created successfully', insertId: result2.insertId, vendor_id, invoiceNum
      });
    });
  });
});
...
app.post('/products/add', (req, res) => {
  ...
  db.query(sql, [invoice_id, product, quantity, unit, rate, total], (err, result) => {
    if (err) return res.status(500).json({ message: 'Database error' });
    return res.status(200).json({
      message: 'Insert successful', insertId: result.insertId, invoice_id: invoice_id, product: product, quantity: quantity, unit: unit, rate: rate, total: total
    })
  })
})
...
app.delete('/products/delete/:id', (req, res) => {
  ...
  db.query('SELECT invoice_id FROM vendor_invoice_items WHERE invoice_item_id = ?', [id], (err1, result1) => {
    if (err1) return res.status(500).json({ message: 'Database error fetching invoice' });
    if (result1.length === 0) return res.status(404).json({ message: 'Product not found' });

    const invoiceId = result1[0].invoice_id;
    db.query('DELETE FROM vendor_invoice_items WHERE invoice_item_id = ?', [id], (err2) => {
      ...
      db.query(
        ...
        [invoiceId, invoiceId],
        (err3) => {
          if (err3) return res.status(500).json({ message: 'Failed to update invoice totals' });
...
app.get('/purchase/total/:invoice_id', (req, res) => {
  ...
  db.query(sql, [invoice_id], (err, result) => {
    if (err) return res.status(500).json({ message: 'Database error' });
    return res.status(200).json({
      message: 'Sum successful', invoice_id: invoice_id, total: result[0].total || 0
    });
  });
});
...
app.post('/purchase/advance/:invoice_id', (req, res) => {
  ...
  db.query(sql, [invoice_id, advance], (err) => {
    if (err) return res.status(500).json({ message: 'Database error' });

    const updateInvoiceSql = `
      UPDATE vendor_invoices
      SET 
        advance_paid = (SELECT COALESCE(SUM(amount),0) FROM vendor_invoice_advances WHERE invoice_id = ?),
        total_amount = ?,
        balance = ? - (SELECT COALESCE(SUM(amount),0) FROM vendor_invoice_advances WHERE invoice_id = ?),
    ...
    db.query(updateInvoiceSql, [invoice_id, total, total, invoice_id, invoice_id, invoice_id], (err2) => {
      ...
      const vendorBalanceSql = `SELECT balance FROM vendor_ledger WHERE vendor_id = ? ORDER BY date DESC LIMIT 1`;
      db.query(vendorBalanceSql, [vendorId], (err3, result3) => {
        if (err3) return res.status(500).json({ message: 'Database error' });

        const lastBalance = result3.length > 0 ? Number(result3[0].balance) : 0;
        ...
        db.query(vendorLedgerSql, [vendorId, 'Payment Made', total, advance, newBalance], (err4) => {
          if (err4) return res.status(500).json({ message: 'Database error' });
...
app.get('/invoices/show', (req, res) => {
  const sql = `
    SELECT 
      vi.invoice_id, vi.invoice_number, vi.total_amount, vi.advance_paid, vi.balance,
      DATE_FORMAT(vi.invoice_date, '%d/%m/%Y') AS invoice_date, v.vendor_name
...
app.get('/vendors/ledger/show/:vendorId', (req, res) => {
  ...
  const sql = `
    SELECT vendor_ledger_id, description, debit, credit, balance, DATE_FORMAT(date, '%d/%m/%Y') AS date
    FROM vendor_ledger WHERE vendor_id = ? ORDER BY date DESC
  `;
...
app.get('/invoices/ledger/show/:invoiceId', (req, res) => {
  ...
  const sql = `
    SELECT v.vendor_name AS vendor, vii.product, vii.quantity, vii.unit, vii.rate, vii.total
...
app.get('/project/ledger/show/:projectId', (req, res) => {
  ...
  const sql = `
    SELECT description, debit, credit, balance, DATE_FORMAT(date, '%d/%m/%Y') AS date
    FROM project_ledger WHERE project_id = ? ORDER BY date DESC, project_ledger_id DESC
  `
...
app.get('/vendors/total-balance', (req, res) => {
  const finalSql = `
    SELECT SUM(final_balance) AS total_balance FROM (
        SELECT CASE WHEN latest_balance IS NOT NULL THEN latest_balance ELSE IFNULL(opening_balance, 0) END as final_balance
        FROM (
            SELECT v.opening_balance, (SELECT balance FROM vendor_ledger WHERE vendor_id = v.vendor_id ORDER BY vendor_ledger_id DESC LIMIT 1) as latest_balance
            FROM vendors v
        ) as raw_data
    ) as final_sum;
  `
...
app.get('/projects/total-receivable', (req, res) => {
  const sql = `
    ...
    FROM (
        SELECT p.project_id, COALESCE((SELECT balance FROM project_ledger pl WHERE pl.project_id = p.project_id ORDER BY pl.project_ledger_id DESC LIMIT 1), 0) AS current_balance
        FROM projects p
    ) AS project_balances;
  `;
  db.query(sql, (err, result) => {
...
app.get('/projects/active-this-year', (req, res) => {
  const sql = `SELECT COUNT(DISTINCT project_id) AS project_count FROM project_ledger WHERE YEAR(date) = YEAR(CURDATE())`;
  db.query(sql, (err, result) => {
    if (err) return res.status(500).json({ message: 'Database error' });
    return res.status(200).json(result[0]);
  });
...
app.get('/dashboard/pending-invoices', (req, res) => {
  const sql = `
    SELECT v.vendor_name, vi.invoice_number, vi.balance, vi.invoice_date 
    FROM vendor_invoices vi JOIN vendors v ON vi.vendor_id = v.vendor_id 
    WHERE vi.balance > 0 ORDER BY vi.invoice_date ASC LIMIT 5
  `;
  db.query(sql, (err, result) => {
...
app.get('/dashboard/recent-activity', (req, res) => {
  const sql = `
    SELECT * FROM (
      SELECT date, description, debit, credit, 'Project' as type FROM project_ledger
      UNION ALL
      SELECT date, description, debit, credit, 'Vendor' as type FROM vendor_ledger
    ) AS combined_activity ORDER BY date DESC LIMIT 10
  `;
  db.query(sql, (err, result) => {
...
app.post('/labor/payments/add', (req, res) => {
  ...
  db.query(getLabor, laborName, (err, result) => {
    ...
    db.query(insertPayment, [labor_id, description, amount, method], (err2, result2) => {
      ...
      const newPayment = {
        labor_payment_id: result2.insertId, description, amount, method, date: new Date().toLocaleDateString('en-GB')
      };
...
// =========================================================================
// 5. REACT FRONTEND ROUTES (MUST BE AT THE VERY BOTTOM)
// =========================================================================

// Serve static files (like your CSS and JS from the build folder)
app.use(express.static(path.join(__dirname, 'build')));

// Bulletproof Catch-All: If no API route matched above, send the React app
// We are using `app.use` instead of `app.get('*')` to avoid ANY path-to-regexp crash.
app.use((req, res) => {
  if (req.method === 'GET') {
